...
lval gc(lval* f) {
  ...
  gcm(dyns);
  for (; f > stack; f--) {
    long* f_as_pointer = (lval*) *f;
    if ((*f & 3) && (f_as_pointer < memory ||
		     f_as_pointer > (memory + memory_size / 4)))
      printf("%lx\n", *f);
    gcm(*f);
  }
  memf = 0;
...
lval make_package(lval* f, const char* s0, const char* s1) {
  return ma(f, 6, 180,
...
int main(int argc, char *argv[]) {
  ...
  stack = malloc(sizeof(lval) * 64 * 1024);
  memset(stack, 0, sizeof(lval) * 64 * 1024);
  g = stack + 5;
  pkg = make_package(g, "CL", "COMMON-LISP");
  for (i = 0; i < 88; i++) {
